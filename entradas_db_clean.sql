...
CREATE TABLE `mesas` (
  ...
  `precio_mesa_completa` decimal(10,2) DEFAULT NULL COMMENT 'Precio fijo al vender la mesa entera',
  `precio_silla_individual` decimal(10,2) DEFAULT NULL COMMENT 'Precio por silla suelta si la mesa no es solo-venta-completa',
  `venta_solo_mesa` tinyint(1) NOT NULL DEFAULT '0' COMMENT '1=solo mesa completa, no sillas sueltas',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
